...
export function createGardenRenderer(host:HTMLElement,hooks:Hooks) {
  ...
  const getTemplate=(species:string)=>{if(!templates.has(species))templates.set(species,loader.loadAsync(`/dino-models/${species}.glb`).then(g=>{g.scene.traverse(o=>{if(o instanceof T.Mesh){prepareToyMotion(o,species);morphBuilds++;}});return g.scene;}));return templates.get(species)!;};
